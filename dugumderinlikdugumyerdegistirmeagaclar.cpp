...
int dugumSayisi(struct node *root)
{
	...
	{
		return dugumSayisi(root->left)+1+ dugumSayisi(root->right); //soldan baþla saðadogru git
	}
}
...
void yerDegistir(struct node *root)
{
	...
	if(root!=NULL) //dügüm boþ deðeri gösterene kadar dolaþ 
	{ //swap islemi yaptýk 
		yerDegistir(root->left);
		yerDegistir(root->right);
		temp = root->left;  //lefti tuttuk 
		root->left = root->right; //left dügümüne right atadik 
		root->right = temp; //right dügümüne lefti atadik 
